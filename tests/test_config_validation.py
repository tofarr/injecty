...
def test_validate_missing_priority():
    ...
    module = create_mock_module(
        "missing_priority", priority=None, configure_func=configure
    )
...
def test_validate_invalid_priority_type():
    ...
    module = create_mock_module(
        "invalid_priority", priority="not_an_int", configure_func=configure
    )
...
def test_validate_non_callable_configure():
    ...
    module = create_mock_module(
        "non_callable_configure", priority=0, configure_func="not_callable"
    )
...
def test_validate_configure_no_parameters():
    ...
    module = create_mock_module(
        "no_params_configure", priority=0, configure_func=configure
    )

    with pytest.raises(ValueError) as excinfo:
        validate_config_module(module)

    assert "invalid 'configure' method: expected at least 1 parameter" in str(
        excinfo.value
    )
...
def test_validate_configure_unusual_parameter_name():
    ...
    module = create_mock_module(
        "unusual_param_name", priority=0, configure_func=configure
    )
